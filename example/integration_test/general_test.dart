...
void main() {
  ...
  group('Initialization', () {
    ...
    test(
      ...
      () {
        ...
        expect(
          FileDownloader().downloaderForTesting.groupStatusCallbacks['test'],
          isNotNull,
        );
        expect(
  ...
  group('Enqueuing tasks', () {
    ...
    testWidgets(
      'enqueue with download speed and time remaining',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        task = DownloadTask(
    ...
    testWidgets(
      'enqueue with non-default group callbacks',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          group: 'test',
    ...
    testWidgets(
      'enqueue with event listener for status updates',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final path = join(
          (await getApplicationDocumentsDirectory()).path,
    ...
    testWidgets(
      'enqueue with event listener and callback for status updates',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(taskStatusCallback: statusCallback);
        final path = join(
    ...
    testWidgets(
      'enqueue with event listener for progress updates',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        task = DownloadTask(
          url:
    ...
    testWidgets(
      'enqueue with event listener, then reset and listen again',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        // Register listener. For testing convenience, we simply route the event
        // to the completer function we have defined
        ...
        expect(
          await FileDownloader().enqueue(task.copyWith(taskId: 'task2')),
          isTrue,
        );
        await statusCallbackCompleter.future;
        expect(statusCallbackCounter, equals(3));
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        expect(
          await fileEqualsLargeTestFile(File(await task.filePath())),
          isTrue,
        );
      },
    );
    ...
    testWidgets(
      'enqueue long Android task that times out',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
    ...
    testWidgets(
      'enqueue with invalid (malformed) url',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
  ...
  group('Queue and task management', () {
    ...
    testWidgets(
      'allTasks and allTaskIds with allGroups set to true',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        ...
        expect(await FileDownloader().allTasks(group: 'non-default'), isEmpty);
        expect(
          () => FileDownloader().allTasks(group: 'some', allGroups: true),
          throwsAssertionError,
        );
        expect(
          () => FileDownloader().allTasks(
    ...
    testWidgets('tasksFinished', timeout: const Timeout(Duration(minutes: 2)), (
      ...
      expect(
        await FileDownloader().tasksFinished(group: 'non-default'),
        isTrue,
      );
      await statusCallbackCompleter.future;
      expect(lastStatus, equals(TaskStatus.complete));
      ...
      expect(await FileDownloader().tasksFinished(), isFalse);
      expect(
        await FileDownloader().tasksFinished(ignoreTaskId: task.taskId),
        isTrue,
      );
      await statusCallbackCompleter.future;
      expect(lastStatus, equals(TaskStatus.complete));
    ...
    testWidgets('taskForId', timeout: const Timeout(Duration(minutes: 2)), (
      ...
      expect(await FileDownloader().taskForId('something'), isNull);
      expect(
        await FileDownloader().taskForId(complexTask.taskId),
        equals(complexTask),
      );
      await statusCallbackCompleter.future;
      expect(statusCallbackCounter, equals(3));
    ...
    testWidgets(
      ...
      (widgetTester) async {
        if (Platform.isAndroid || Platform.isIOS) {
          ...
          expect(
            await FileDownloader().getRequireWiFiSetting(),
  ...
  group('Convenience downloads', () {
    testWidgets(
      ...
      (widgetTester) async {
        ...
        expect(
          result.responseHeaders?['server']?.startsWith('Werkzeug'),
          isTrue,
        );
    ...
    testWidgets(
      'multiple download with futures',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
    ...
    testWidgets(
      'batch download with batch callback',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final tasks = <DownloadTask>[];
        final docDir = (await getApplicationDocumentsDirectory()).path;
    ...
    testWidgets(
      'batch download with task callback',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        ...
        expect(
          statusCallbackCounter,
          equals(15),
        ); // 3 attempts + 2 retry attempts, each 3
        expect(progressCallbackCounter, greaterThanOrEqualTo(10));
      },
    );

    testWidgets(
      'batch download with onElapsedTime',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final tasks = <DownloadTask>[
    ...
    testWidgets(
      'convenience download with callbacks',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
    ...
    testWidgets(
      'parallel convenience downloads with callbacks',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        var a = 0, b = 0, c = 0;
        ...
        successResult.then(
          (value) => expect(value.status, equals(TaskStatus.complete)),
        );
        successResult2.then(
          (value) => expect(value.status, equals(TaskStatus.complete)),
        );
        failingResult.then(
          (value) => expect(value.status, equals(TaskStatus.failed)),
        );
    ...
    testWidgets(
      'simple parallel convenience downloads with callbacks',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final failTask = DownloadTask(
          url: urlWithFailure,
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        await FileDownloader().configure(
          globalConfig: (Config.skipExistingFiles, 0),
        ); // 0 MB
        ...
        await FileDownloader().configure(
          globalConfig: (Config.skipExistingFiles, 1),
        ); // 1 MB
        result = await FileDownloader().download(task);
        expect(
          result.status,
  ...
  group('Retries', () {
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        if (retriedTask != null) {
          expect(retriedTask.retriesRemaining, lessThan(retriedTask.retries));
          expect(
            await FileDownloader().cancelTasksWithIds([retriedTask.taskId]),
            isTrue,
          );
          await statusCallbackCompleter.future;
          expect(lastStatus, equals(TaskStatus.canceled));
    ...
    testWidgets(
      'retry progress updates with cancellation',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    testWidgets(
      'queue management allTasks with retries',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(taskStatusCallback: statusCallback);
        expect(await FileDownloader().enqueue(retryTask), isTrue);
        ...
        final nonRetryTasksBeforeWait = await FileDownloader().allTasks(
          includeTasksWaitingToRetry: false,
        );
        ...
        final nonRetryTasksAfterWait = await FileDownloader().allTasks(
          includeTasksWaitingToRetry: false,
        );
    ...
    testWidgets(
      'queue management: taskForId with retries',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(taskStatusCallback: statusCallback);
        expect(await FileDownloader().enqueue(retryTask), isTrue);
        ...
        expect(
          await FileDownloader().taskForId(retryTask.taskId),
          equals(retryTask),
        );
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        await someProgressCompleter.future;
        expect(
          await FileDownloader().taskCanResume(task),
          isFalse,
        ); // allowPause not set
        print('FAIL TASK NOW!');
        await statusCallbackCompleter.future;
        if (lastStatus == TaskStatus.failed) {
          ...
          expect(
            await FileDownloader().downloaderForTesting.getResumeData(
              task.taskId,
            ),
            isNotNull,
          );
  ...
  group('DownloadTask with POST request', () {
    testWidgets(
      'post DownloadTask with post is empty body',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final task = DownloadTask(
          url: postTestUrl,
    ...
    testWidgets(
      'post DownloadTask with post is String',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final task = DownloadTask(
          url: postTestUrl,
    ...
    testWidgets(
      'post DownloadTask with post is Uint8List',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final task = DownloadTask(
          url: postTestUrl,
    ...
    testWidgets(
      'post DownloadTask with post is JsonString',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final task = DownloadTask(
          url: postTestUrl,
    ...
    testWidgets(
      'post DownloadTask with post is Map',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final task = DownloadTask(
          url: postTestUrl,
    ...
    testWidgets(
      'post DownloadTask with post is List',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final task = DownloadTask(
          url: postTestUrl,
    ...
    testWidgets(
      'post DownloadTask with post is invalid type',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        expect(
          () => DownloadTask(
  ...
  group('Request', () {
    ...
    testWidgets(
      'post request with post is empty body',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final request = Request(
          url: postTestUrl,
    ...
    testWidgets(
      'post request with post is String',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final request = Request(
          url: postTestUrl,
    ...
    testWidgets(
      'post request with post is JsonString',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final request = Request(
          url: postTestUrl,
    ...
    testWidgets(
      'post request with post is List',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final request = Request(
          url: postTestUrl,
    ...
    testWidgets(
      'post request with post is invalid type',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        expect(
          () => Request(
    ...
    testWidgets(
      'get request with server error, no retries',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
    ...
    testWidgets(
      'get request with server error, with retries',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
    ...
    testWidgets(
      'get request with malformed url error, no retries',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
  ...
  group('Basic upload', () {
    ...
    testWidgets(
      'upload binary file partially bytes=2-4',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    testWidgets(
      'upload binary file partially bytes=2-',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    testWidgets(
      'upload binary file partially invalid headers',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        final result = await FileDownloader().upload(
          uploadTask.copyWith(fields: fields),
        );
        expect(result.status, equals(TaskStatus.complete));
    ...
    testWidgets(
      'enqueue multipart with fields that have multiple values',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    testWidgets(
      'enqueue binary file, then cancel',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    testWidgets(
      'upload task creation with errors',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        expect(
          () => UploadTask(
    ...
    testWidgets(
      'Create UploadTask from File',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final pathToFile = await uploadTask.filePath();
        final file = File(pathToFile);
        final taskFromFile = UploadTask.fromFile(
          file: file,
          url: uploadTask.url,
        );
  ...
  group('Convenience uploads', () {
    testWidgets(
      ...
      (widgetTester) async {
        ...
        expect(
          result.responseHeaders?['server']?.startsWith(serverName),
          isTrue,
        );
        expect(result.responseStatusCode, equals(200));
      },
    ...
    testWidgets(
      'batch upload with onElapsedTime',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final tasks = <UploadTask>[
          uploadTask,
        ...
        var ticks = 0;
        final result = await FileDownloader().uploadBatch(
          tasks,
          onElapsedTime: (elapsed) => ticks++,
          elapsedTimeInterval: const Duration(milliseconds: 20),
        );
    ...
    testWidgets(
      'convenience upload with callbacks',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
  ...
  group('MultiUpload', () {
    ...
    testWidgets(
      'upload 2 files with full file path',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
  ...
  group('Cancellation', () {
    ...
    testWidgets(
      'immediately cancel a task that fails immediately',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(taskStatusCallback: statusCallback);
  ...
  group('Tracking', () {
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        final record3 = await FileDownloader().database.recordForId(
          task.taskId,
        );
  ...
  group('Pause and resume', () {
    ...
    testWidgets(
      'pause and resume task with ? as filename',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    testWidgets(
      'pause and resume with invalid ETag',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        ...
        if (!Platform.isIOS) {
          ...
          final newResumeData = ResumeData(
            task,
            resumeData!.data,
    ...
    testWidgets(
      'Pause and resume a convenience download',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        task = DownloadTask(
          url: urlWithContentLength,
    ...
    testWidgets(
      'Pause and resume a task with a Range header',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        const rangeStart = 10;
  ...
  group('Fail background channel', () {
    testWidgets(
      ...
      (widgetTester) async {
        if (Platform.isAndroid || Platform.isIOS) {
          ...
          expect(
            (await downloader.getResumeData(task.taskId))?.taskId,
            equals(task.taskId),
          );
          expect((await downloader.getPausedTask(task.taskId)), equals(task));
          // confirm all popped data is gone
          final resumeDataMap = await downloader.popUndeliveredData(
            Undelivered.resumeData,
          );
          expect(resumeDataMap.length, equals(0));
          final statusUpdateMap = await downloader.popUndeliveredData(
            Undelivered.statusUpdates,
          );
          expect(statusUpdateMap.length, equals(0));
          final progressUpdateMap = await downloader.popUndeliveredData(
    ...
    testWidgets(
      'Local storage for taskStatusUpdate - ok download',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        if (Platform.isAndroid || Platform.isIOS) {
          final downloader = FileDownloader().downloaderForTesting;
          ...
          expect(
            taskStatusUpdate.responseHeaders?.containsKey('content-length'),
            isTrue,
          );
          ...
          await Future.delayed(const Duration(milliseconds: 500));
          await downloader
              .retrieveLocallyStoredData(); // triggers status update
          downloader.retrievedLocallyStoredData = false; // reset for testing
          await Future.delayed(const Duration(milliseconds: 500));
          expect(taskStatusUpdate.status, equals(TaskStatus.complete));
          expect(taskStatusUpdate.exception, isNull);
    ...
    testWidgets(
      'Local storage for taskStatusUpdate - failed download',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        if (Platform.isAndroid || Platform.isIOS) {
          final downloader = FileDownloader().downloaderForTesting;
  ...
  group('Notifications', () {
    ...
    testWidgets('openFile', timeout: const Timeout(Duration(minutes: 2)), (
      ...
      if (!Platform.isAndroid) {
        ...
        final filePath = await task.filePath();
        await File(filePath).rename(
          join(dirname(filePath), '${basenameWithoutExtension(filePath)}.txt'),
        );
        task = task.copyWith(filename: '1MB-test.txt');
        success = await FileDownloader().openFile(task: task);
        expect(success, isTrue);
        success = await FileDownloader().openFile(
        ...
        task = task.copyWith(filename: '1MB-test.txt');
        final newFilename = await FileDownloader().moveToSharedStorage(
          task,
  ...
  group('Directories', () {
    test('Print directory names', timeout: const Timeout(Duration(minutes: 2)), () async {
      print(
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        print(
          await FileDownloader().configure(
            androidConfig: (Config.useExternalStorage, Config.never),
          ),
        );
      },
    );
  ...
  group('Shared storage', () {
    ...
    test(
      '[*] try to move text file to images -> error',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
    ...
    test(
      'move while overriding mime type',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        if (Platform.isAndroid) {
          var filePath = await task.filePath();
    ...
    test(
      'move file to shared storage - all types',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
  ...
  group('Exception details', () {
    ...
    testWidgets(
      'convenience download for httpResponse 403',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        task = DownloadTask(url: urlWithFailure, filename: 'test');
        final result = await FileDownloader().download(task);
    ...
    testWidgets(
      'fileSystem: File to upload does not exist',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        if (!Platform.isIOS) {
          FileDownloader().registerCallbacks(
  ...
  group('Content-disposition', () {
    testWidgets(
      ...
      (widgetTester) async {
        ...
        task = DownloadTask(
          url: urlWithoutContentLength,
        ); // has url last path segment
        expect(
          await downloader.testSuggestedFilename(task, ''),
          equals('1MB-test.bin'),
        );
      },
    );
    ...
    testWidgets(
      'DownloadTask withSuggestedFilename',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        await deleteOld5MbDownloads();
        task = DownloadTask(url: urlWithContentLength);
    ...
    testWidgets(
      'downloadTask with ? for filename',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        await deleteOld5MbDownloads();
        task = DownloadTask(
    ...
    testWidgets(
      'parallelDownloadTask with ? for filename',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        await deleteOld5MbDownloads();
    ...
    test(
      'uploads with different content-disposition headers',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        ...
        for (final entry in cdMap.entries) {
          ...
          if (entry.key != null) {
            myTask = uploadTask.copyWith(
              headers: {'Content-Disposition': entry.key!},
            );
          } else {
            myTask = uploadTask;
          }
  ...
  group('Range and Content-Length headers', () {
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        if (Platform.isLinux || Platform.isMacOS || Platform.isWindows) {
          ...
          expect(
            getContentLength({'Content-Length': '123'}, task),
            equals(123),
          );
          expect(
            getContentLength({'content-length': '123'}, task),
    ...
    testWidgets(
      'Range header in download request',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        const rangeStart = 10;
  ...
  group('Cookies', () {
    testWidgets(
      'cookie from live set-cookie response header',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final request = Request(
          url:
  ...
  group('Priority and TaskQueue', () {
    ...
    testWidgets(
      'One high priority task among regular ones',
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        final tasks = <DownloadTask>[];
        for (var n = 1; n < 40; n++) {
    ...
    testWidgets(
      ...
      (widgetTester) async {
        ...
        ); // allow for second task to start if not paused
  ...
  group('HoldingQueue', () {
    ...
    test(
      'holdingQueue allTaskIds and allTasks',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        expect(
          (await FileDownloader().configure(
        ...
        expect(
          await FileDownloader().allTaskIds(group: 'non-existent'),
          isEmpty,
        );
        await waitForTasksToClear();
      },
      ...
      () async {
        for (final baseDirectoryEnum in BaseDirectory.values) {
          ...
          if (Platform.isWindows && baseDirectoryEnum == BaseDirectory.root) {
            ...
            expect(
              await Task.baseDirectoryPath(baseDirectoryEnum),
              equals(dir),
            );
          }
        }
  ...
  group('FilePath', () {
    ...
    test(
      'returns correct file path for task using file URI',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        final task = UriUploadTask(
          url: urlWithoutContentLength,
          fileUri: Uri.file('/my/file/uri.txt'),
        );
        final path = await task.filePath();
    ...
    test(
      'returns correct file path for UriUploadTask with URI and filename',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
    ...
    test(
      'returns correct file path for UriDownloadTask with URI and filename',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        ...
        expect(
          task.fileUri!.toFilePath(windows: Platform.isWindows),
    ...
    test(
      'throws assertion error for task using non-file URI',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        final task = UriUploadTask(
          url: urlWithoutContentLength,
    ...
    test(
      'returns correct path for task using root directory',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        final task = DownloadTask(
          url: urlWithoutContentLength,
  ...
  group('DataTask', () {
    test(
      ...
      () async {
        ...
        final lastUpdate = await completer.future.timeout(
    ...
    test(
      ...
      () async {
        ...
        ); // without 'POST' will fail
        expect(await FileDownloader().enqueue(t), isTrue);
        var lastUpdate = await completer.future.timeout(
          const Duration(seconds: 30),
        );
        ...
        final t2 = DataTask(
          ...
          headers: dataTaskHeaders,
          httpRequestMethod: 'POST',
        ); // with 'POST' will succeed
        expect(await FileDownloader().enqueue(t2), isTrue);
        lastUpdate = await completer.future.timeout(
          const Duration(seconds: 30),
        );
        expect(lastUpdate.status, equals(TaskStatus.complete));
