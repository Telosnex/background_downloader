...
void main() {
  ...
  group('Binary uploads', () {
    test(
      ...
      () async {
        ...
        final dummy = DownloadTask(
          url: uploadTestUrl,
          filename: uploadFilename,
        );
        final uri = await FileDownloader().uri.moveToSharedStorage(
          dummy,
        ...
        expect(
          resultTask.filename.substring(0, 6),
    ...
    test(
      'upload binary file using incorrect Android content URI',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        final task = UriUploadTask(
          url: uploadBinaryTestUrl,
        ...
        expect(
          ...
          equals('TaskFileSystemException'),
        );
      },
    ...
    test(
      ...
      () async {
        ...
        expect(
          (lastTaskWithStatus as UriUploadTask).fileUri!.scheme,
          equals('file'),
        );
  ...
  group('Multipart uploads', () {
    test(
      'enqueue multipart file using Android content URI',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
    ...
    test(
      'upload multipart file using incorrect Android content URI',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        final task = UriUploadTask(
    ...
    test(
      'upload single multipart file using file URI',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
    ...
    test(
      ...
      () async {
        ...
        final dummy2 = DownloadTask(
          url: uploadTestUrl,
          filename: uploadFilename2,
        );
        final uri2 = await FileDownloader().uri.moveToSharedStorage(
          dummy2,
          SharedStorage.downloads,
        );
        print('uri1=$uri\nuri2=$uri2');
  ...
  group('Downloads with file URI', () {
    test(
      'download file without filename',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        ...
        expect(
          task.fileUri,
          equals(Uri.parse('$directoryUri/${task.filename}')),
        );
    ...
    test(
      'download file with suggested filename',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        final directory = await getApplicationDocumentsDirectory();
        final directoryUri = Uri.file(directory.path);
    ...
    test(
      'Download and pause/resume file with suggested filename',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        FileDownloader().registerCallbacks(
          taskStatusCallback: statusCallback,
  ...
  group('Other utils', () {
    testWidgets(
      ...
      (widgetTester) async {
        ...
        print('URI: $uri');
        final result = await FileDownloader().uri.openFile(
          uri!,
          mimeType: 'text/plain',
        );
  ...
  group('File utils', () {
    test(
      'move task to shared storage with file URI',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        ...
        final fileUri = Uri.file(filePath);
        final uri = await FileDownloader().uri.moveFileToSharedStorage(
          fileUri,
          SharedStorage.downloads,
        );
        print('Uri is $uri');
    ...
    testWidgets(
      'path in shared storage with file URI',
      // note: moved file is not deleted in this test
      timeout: const Timeout(Duration(minutes: 2)),
      (widgetTester) async {
        await FileDownloader().download(task);
        ...
        if (uri!.scheme == 'file') {
          ...
          final uri2 = await FileDownloader().uri.pathInSharedStorage(
            uri,
            SharedStorage.downloads,
          );
          print('Uri2 is $uri2');
          ...
          expect(
            () async => await FileDownloader().uri.pathInSharedStorage(
              uri,
              SharedStorage.downloads,
            ),
    ...
    test(
      ...
      () async {
        ...
        final newDirUri = await FileDownloader().uri.createDirectory(
          tempDirUri,
          testDir,
        );
        ...
        final newSubDirUri = await FileDownloader().uri.createDirectory(
          newDirUri,
          testSubDir,
        );
        ...
        final multiLevelDirUri = await FileDownloader().uri.createDirectory(
          tempDirUri,
          multiLevelDirName,
        );
    ...
    test(
      ...
      () async {
        ...
        // Copy the file.
        final resultUri = await FileDownloader().uri.copyFile(
          sourceUri,
          destUri,
        );
        // Verify the result.
    ...
    test(
      ...
      () async {
        ...
        final destUri = Uri.file(destFilePath);
        // Move the file.
        final resultUri = await FileDownloader().uri.moveFile(
          sourceUri,
          destUri,
        );
        // Verify the result.
    ...
    test(
      'copyFile with file:// URIs and String destination',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        // Create a source file.
        final directory = await getTemporaryDirectory();
    ...
    test(
      'moveFile with file:// URIs and String destination',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        // Create a source file.
        final directory = await getTemporaryDirectory();
    ...
    test(
      'copyFile with file:// URIs and File destination',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        // Create a source file.
        final directory = await getTemporaryDirectory();
    ...
    test(
      'moveFile with file:// URIs and File destination',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        // Create a source file.
        final directory = await getTemporaryDirectory();
    ...
    test(
      'copyFile and moveFile throw with invalid destination type',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        // Create a source file.
        final directory = await getTemporaryDirectory();
        ...
        expect(
          () async => await FileDownloader().uri.copyFile(
            sourceUri,
            invalidDestination,
          ),
        ...
        expect(
          () async => await FileDownloader().uri.moveFile(
            sourceUri,
            invalidDestination,
          ),
          throwsA(isA<ArgumentError>()),
    ...
    test(
      'copyFile from content URI to file URI on Android',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        ...
        final contentUri = await FileDownloader().uri.moveToSharedStorage(
          dummy,
          SharedStorage.downloads,
        );
    ...
    test(
      'moveFile from content URI to file URI on Android',
      timeout: const Timeout(Duration(minutes: 2)),
      () async {
        // Move a test file to shared storage to obtain a content:// URI.
        final dummy = DownloadTask(
        ...
        // Verify content.
        final bytes = await FileDownloader().uri.getFileBytes(
          contentUri,
        ); // Get original content
        expect(bytes, isNotNull);
        final destFile = File.fromUri(destUri);
        expect(
          await destFile.readAsBytes(),
          bytes,
        ); //compare with destination content
        await destFile.delete();
      },
      skip: !Platform.isAndroid,
    );
