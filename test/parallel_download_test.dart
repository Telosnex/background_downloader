...
void main() {
  test('createChunks', () {
    ...
    expect(
      () => createChunks(task, {'content-length': '100'}),
      throwsA(const TypeMatcher<StateError>()),
    );
    expect(
      () => createChunks(task, {'accept-ranges': 'bytes'}),
      throwsA(const TypeMatcher<StateError>()),
    );
    expect(
      () => createChunks(task, {
  ...
  test('updates', () {
    ...
    final progressUpdate = updateChunkProgress(
      TaskProgressUpdate(chunks.first.task, 0.5),
    );
  ...
  test('json chunks', () {
    ...
    final List<Chunk> decodedChunks = List.from(
      jsonDecode(chunksJson, reviver: Chunk.listReviver),
    );
